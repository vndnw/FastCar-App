...
function UserDetail() {
    ...
    return (
        <>
            <Meta
                title={`User Details - ${user?.fullName || user?.email || 'User'} - Admin Dashboard`}
                description={`Detailed view of user account information and activity for ${user?.fullName || user?.email || 'this user'}`}
            />
            <div>
                {/* Header */}
